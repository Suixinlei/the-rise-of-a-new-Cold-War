...
function P.ProductionCheck(voType, voProductionData)
	...
	-- Which sort of units an AI would even consider to buy?
	if voType.SubType == "Armor"		-- Main battle tanks
	or voType.SubType == "Mech"			-- Mechanised vehicles
	or voType.SubType == "Support"		-- AA, AT, SP.. & etc
	or voType.SubType == "Escort"		-- Green water warships
	or voType.SubType == "Cruiser"		-- Blue water warships
	or voType.SubType == "Submarine"	-- Submarines
	or voType.SubType == "Transport"	-- Transports
	or voType.SubType == "Carrier Plane"-- Carrier Air Group
	or voType.Type == "Air"				-- Any air units
	then
...
function P.CalculateCost(voBuyerInfo, voSellerInfo, voSubUnit)
	...
	if voBuyerInfo.Faction == voSellerInfo.Faction and voBuyerInfo.HasFaction then
		if voBuyerInfo.Country:IsAtWar() then
			liCost = (liICCode * liBuildTime) * 0.0025 -- 25% of 1 percent
		else
			liCost = (liICCode * liBuildTime) * 0.005 -- 50% of 1 percent
		end
