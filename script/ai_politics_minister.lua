...
--
local _ABOLISHED_MILITARY_ = 1
local _VOLUNTEER_ARMY_ = 2
local _PARTIAL_DRAFT_ = 3
local _MILITARY_SERVICE_ = 4
local _UNIVERSAL_MILITARY_SERVICE_ = 5

local _PEACE_TIME_ = 6
local _LOW_MOBILIZATION_ = 7
local _L_MOBILIZATION = 8
local _HIGH_ALERT_ = 9
local _WAR_TIME_ = 10

local _FULL_PROTECTION_ = 11
local _MULTIPLE_LAWS_ = 12
local _SOME_LAWS_ = 13
local _NO_LAWS_ = 14

local _OPEN_SOCIETY_ = 15
local _RESTRICTIONS_ = 16
local _POLICE_STATE_ = 17
local _TOTALITARIANISM_ = 18

local _SIX_MONTHS_ = 19
local _TWELVE_MONTHS = 20
local _EIGHTEEN_MONTHS_ = 21
local _TWOFOUR_MONTHS_ = 22

local _LAIS_FAIR_ = 23
local _LIMIT_INTERVENTION_ = 24
local _PARTIAL_CONTROL_ = 25
local _STATE_CONTROL_ = 26

local _NO_HEALTHCARE_ = 27
local _AID_GROUPS_ = 28
local _PRIVATE_CARE_ = 29
local _PUBLIC_CARE_ = 30


local _HEAD_OF_STATE_ = 1
local _HEAD_OF_GOVERNMENT_ = 2
local _MINISTER_OF_FOREIGN_ = 3
local _MINISTER_OF_FINANCE_ = 4
...
function Laws(minister)
	...
	for loGroup in CLawDataBase.GetGroups() do
		...
		elseif (loGroupName == "civil_laws") then
			loNewLaw = CivilLaws(ministerTag, ministerCountry, loCurrentLaw)

		elseif (loGroupName == "conscription_policy") then
			loNewLaw = ConscriptionPolicy(ministerTag, ministerCountry, loCurrentLaw)
			
		elseif loGroupName == "economic_policy" then
			loNewLaw = EconomicPolicy(ministerTag, ministerCountry, loCurrentLaw)
			
		elseif loGroupName == "mobilisation_level" then
			loNewLaw = MobilisationLevel(ministerTag, ministerCountry, loCurrentLaw)
			
		elseif loGroupName == "environmental_policy" then
			loNewLaw = EnvironmentalPolicy(ministerTag, ministerCountry, loCurrentLaw)
			
		elseif loGroupName == "service_length" then
			loNewLaw = ServiceLength(ministerTag, ministerCountry, loCurrentLaw)
		
		elseif loGroupName == "healthcare" then
			loNewLaw = Healthcare(ministerTag, ministerCountry, loCurrentLaw)
...
-- Law sub-methods
--################
function ConscriptionPolicy(ministerTag, ministerCountry, voCurrentLaw)
...
function CivilLaws(ministerTag, ministerCountry, voCurrentLaw)
...
function EconomicPolicy(ministerTag, ministerCountry, voCurrentLaw)
	local liIndex = voCurrentLaw:GetIndex() + 1
	local loNewLaw = nil
...
function MobilisationLevel(ministerTag, ministerCountry, voCurrentLaw)
	local liIndex = voCurrentLaw:GetIndex() + 1
	local loNewLaw = nil
...
function EnvironmentalPolicy(ministerTag, ministerCountry, voCurrentLaw)
	local liIndex = voCurrentLaw:GetIndex() + 1
	local loNewLaw = nil
...
function Healthcare(ministerTag, ministerCountry, voCurrentLaw)
	local liIndex = voCurrentLaw:GetIndex() + 1
	local loNewLaw = nil
...
function ServiceLength(ministerTag, ministerCountry, voCurrentLaw)
	local liIndex = voCurrentLaw:GetIndex() + 1
	local loNewLaw = nil
